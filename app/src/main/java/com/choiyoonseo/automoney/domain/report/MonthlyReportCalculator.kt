...
data class MonthlyReport(
    ...
    val categoryExpenseWonByName: Map<String, Long>
)
...
class MonthlyReportCalculator {
    fun calculate(transactions: List<MoneyTransaction>): MonthlyReport {
        ...
        val categoryNameTotals = expenseTransactions
            .mapNotNull { transaction ->
                transaction.categoryDisplayName()?.let { name -> name to transaction.effectiveExpenseWon() }
            }
            .groupBy({ it.first }, { it.second })
            .mapValues { (_, amounts) -> amounts.sum() }
        ...
        return MonthlyReport(
            ...
            categoryExpenseWonByName = categoryNameTotals
        )
    }
}
