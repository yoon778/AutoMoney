...
## Notification Field Selection Fix — 2026-07-27

- KB스타뱅킹 실기기 알림의 `제목: 출금 10,000원` + `본문: 체크카드출금 10,000 잔액52,980` 분리 형식 복원
- `잔액` 포함 자체가 아니라 잔액 문구 제거 후 거래 행동어가 남는지 판정
- 체크카드 출금은 계좌이체가 아닌 저신뢰 지출 검토로 저장
- 현재 제목의 확정 거래는 순수 잔액 본문보다 우선하되 광고·실패·예정 및 과거 `bigText` fallback 차단 유지
- 등록 은행 공통 파서와 사용자 허용 앱 Generic 파서에 동일 정책 적용
- 전체 unit test와 debug APK 빌드 통과
