...
## Tasks
- [ ] **T1: DAO 삭제 쿼리** — `AssetDao`에 `@Query("DELETE FROM fixed_expenses WHERE id = :id")` / `@Query("DELETE FROM monthly_plan_items WHERE id = :id")` 추가. 마이그레이션 불필요(스키마 변화 없음).
- [ ] **T2: Repository 삭제 경로** — `AssetRepository`에 `deleteFixedExpense(id: Long)` / `deleteMonthlyPlanItem(id: Long)` 추가, `RoomAssetRepository` 구현.
- [ ] **T3: UI 삭제 버튼** — `AssetsScreen` 고정지출/월계획 목록의 `AssetRow`에 `actionLabel="삭제"` + `onAction` 연결. 미리보기(repository null)면 안내 메시지. 삭제 후 스낵/칩 메시지.
- [ ] **T4 (후순위): 항목 수정** — 필요 시 별도 계획. 지금은 삭제+재추가로 대체.
- [ ] **검증:** `:app:compileDebugKotlin` + 에뮬레이터에서 추가→삭제 동작 확인.
